...
In the last of this section, we mainly discussed the judgement of chaos in 1 dimension. Here we will focus on the Lyapunov exponent in high dimension. Obviously, in high dimension, the $\delta(t)$ function we used in Discussion \ref{Lyapunov-exp-discussion} will be a $n$ dimension vector $\mathbf \delta(t)$ and we have
$$
\mathbf \delta(t) = \exp(\lambda t)\mathbf\delta(0)
$$
However, we know that in high dimension problem it is difficult to find every orbit convergence together and in most time some of them decreasing and the others are increasing. So we want to know the Lyapunov exponent for every dimension rather than in all dimension. Note that the exponent of a matrix still a matrix, let $\lambda$ to $\Lambda$ which is a $n\times n$ dimension matrix, then
$$
\mathbf \delta(t) = \exp(\Lambda t)\mathbf\delta(0) = \mathbf M \mathbf\delta(0)
$$
