...
Finally, we try to analysis the calculation of these fractal. In analysis we know that \textbf{measure} is a describe of area, also we define the Lebesgue outer measure in follow
...
In this subsection, we mainly focus on the area based on the cube. 
...
$$
||\delta_n|| = \exp(\lambda t) ||\delta_0||
$$
$$
n\rightarrow \infty, \mathbf ||\delta_n|| = \exp(\lambda n)\mathbf||\delta_0||
$$
$$
\Rightarrow \lambda = \lim_{n \rightarrow \infty}{1\over n}\log{\mathbf||\delta_n|| \over \mathbf ||\delta_0||}
= \lim_{n \rightarrow \infty}\log\left[\mathbf||\delta_n|| \over \mathbf ||\delta_0||\right]^{1\over n}
$$
...
$$
\forall i = 1, 2, \ldots m, \lambda_i = \lim_{n \rightarrow \infty}\log\left[\mathbf||\omega^{(i)}_n|| \over \mathbf ||\omega^{(i)}_0||\right]^{1\over n}
$$
...
So we have 
$$
\Lambda = \lim_{n \rightarrow \infty}\left[\log\left|\prod_{i = n}^{0}P_i\right|^{1\over n}\right]
$$

And we can calculate the Lyapunov spectrum with the formula above, however if we try to calculate the formula, we found that is difficult to calculate the $P = \prod_{i = n}^{0}P_i$ of formula above. Based on the discussion of sink and source, we know that if the eigenvalue of the matrix is greater than 1, then the matrix is disconvergence, or disconvergence for a line. So to solve the Lyapunov spectrum of high dimension function, we need some basic method in orthogonal decomposition.

Firstly, we have these conclusions
\begin{conclusion}\textbf{[i]} If we have a orthogonal basis, then based on the Gram-Schmidt method, we have a group of normal orthogonal basis.
\begin{algorithm}[H]
...
\end{algorithm}
  ...
  \noindent\textbf{[ii]} Consider a matrix $Y = XN$ where $N$ is normal orthogonal decomposition matrix
\end{conclusion}


{\color{blue}
\begin{conclusion} \textbf{Calculation of Lyapunov spectrum in high dimension problem}
...
$\text{Lyapunov spectrum} = (0.44004674, 0.01101559), \text{sum} = 0.4510623256923707$
\end{example}
...
\end{center}
\end{figure}
$\text{Lyapunov spectrum} = (4.00544221, -2.24674614, -15.42821896), \text{sum} = -13.669522889292935$
\begin{figure}[H]
\begin{center}
...
\end{center}
\end{figure}
$\text{Lyapunov spectrum} = (4.8857278, -1.61501712, -18.23236088), \text{sum} = -14.961650200700317$
\end{example}
...
\end{center}
\end{figure}
$\text{Lyapunov spectrum} = (0.65174245, 0.04364673, -10.32448076), \text{sum} = -9.629091584454333$
\end{example}
...
\end{center}
\end{figure}
$\text{Lyapunov spectrum} = (1.0913024, -0.99141093), \text{sum} = 0.09989146956531447$
\end{example}
