...
$$
To analysis the problem with Lyapunov exponent, we need to calculate the partical differential of the function.
$$
{\partial\tau \over \partial x} = {-2C_3x \over (1 + x^2 + y^2)^2} \ \ \ \ 
{\partial\tau \over \partial y} = {-2C_3y \over (1 + x^2 + y^2)^2} \ \ \ \  
...
$$
And finally, if we consider the Jacobian matrix $J$, then
$$
J = C_2 \cdot
\left[\begin{array}{cc}
    \cos \tau & -\sin \tau \\
...
Finally, 
$$
J = {-2C_2C_3 \over (1 + x^2 + y^2)^2} \cdot
\left[\begin{array}{cc}
    \cos \tau & -\sin \tau \\
    \sin \tau & \cos \tau  
\end{array}\right]
...
\left[\begin{array}{cc}
    1-xy & -y^2 \\
    x^2 & 1+xy  
\end{array}\right] \left(\tau = C_1 - {C_3 \over 1+x^2+y^2}\right)
$$
