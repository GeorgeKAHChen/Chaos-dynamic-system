...
= \ldots = \prod_{i = n - 1}^{0}J_i\mathbf\delta_0
$$
and 
$$
M_n = J_{n-1}J_{n-2} \ldots J_1 J_0 
$$
...
$$
$$
\lambda_i = \lim_{n \rightarrow \infty}{1\over n}\ln\alpha_{ni}
%\lambda_i = \lim_{n \rightarrow \infty}{1\over n}\sum_{i = 1}^{n}\ln\alpha_{ni}
$$
where $\alpha_{ni}$ is the eigenvalue of $M_n$ and the order of eigenvalue $i$ is confirmed in follow
...
\begin{example}\textbf{Lyapunov spectrum of Lorenz system}
...
\dot y & = & x(\rho - z) - y \\
\dot z & = & xy - \beta z
\end{array}\right. \text{ and the Jacobian matrix is } J(x, y, z) = \left[\begin{array}{lll}
-\sigma     & \sigma    & 0         \\
(\rho-z)    & -1        & -x        \\
y           & x         & -\beta
\end{array}\right]
$$
We try to solve the problem numerically. Let initial value at $t_0$ is $\left(x(t_0), y(t_0), z(t_0)\right)^T = (x_0, y_0, z_0)^T$ and find a group of value every $\Delta t$, then we have
$$
(x_n, y_n, z_n)^T = \left(x(t_0 + n\Delta t), y(t_0 + n\Delta t), z(t_0 + n\Delta t)\right)
$$
...
\end{property}
\text{The proof of this property is simple because we know that for a certain interval $(a, b)$, the measure }$\mu\left((a, b)\right) = b-a$ \text{which also be called as \textbf{Borel measure}. And we can just calculate the measure of $G$ with limitation.}
...
\\\noindent \textbf{[i-1]} if coin comes up heads, then move the point $x_{i-1}$ to $x_i = {2\over 3} x_{i-1}$
\\\noindent \textbf{[i-2]} if coin comes up tails, then move the point $x_{i-1}$ to $x_i = {1 \over 3}(1 + 2x_{i-1})$
\end{example}
