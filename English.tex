...
%However, if we want to analysis the change of map during the iteration, we may need to analysis every Lyapunov exponent and that is a little difficult. So in this part, we will introduce the Lyapunov dimension, which can describe the system is convergence or divergence. 

%We know that if Lyapunov exponent is greater than 0, then the vector in that dimension will be longer than original one. In the lase subsection, we used the circle neigibourhood to explain the necessity of improve the 1-dim vector to m-dim vectors. Now we consider a box neigibourhood which still have the center of initial point. Obviously, the volume of this neigibourhood is multiply norm of every vector which has been used in calculation of Lyapunov spectrum. And if the volume is increasing during the iteration, then the system will used more and more area and vice versa. 
...
We can check the conclusion with the volumn of the map.
\begin{theorem}\textbf{Volumn of the map}
\\\noindent Consider a map $f = (f_1, f_2, \ldots, f_m)$ in $\mathcal R^m$ the volumn of the map can be defined in follows
$$
V = \sum_{i = 1}^{m}{\partial \dot f_m\over \partial x_m}
$$
and the sum of the Lyapunov spectrum is equal to the volumn $V$.
\end{theorem}
