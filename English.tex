...
To analysis this Lorenz model, Michel Henon simplified model with Poincare section, called Henon map, which is the main topic we focus on in this section. 
...
\begin{theorem}\textbf{Similar matrices and Jordan normal form}
\\\noindent Consider a matrix $A_{n\times n}$.
\\\noindent \textbf{[i]} For another matrix $B_{n\times n}$, $\exists P$ s.t. $A = P^{-1} B P$, then called A and B are similar.
\\\noindent \textbf{[ii]} If $J$ is Jordan form of $A$, then $\exists P = (\mathbf v_1, \mathbf v_2, \ldots \mathbf v_n)$ s.t. $A = P^{-1} J P$ where $\mathcal v_i$ are eigenvector (or based on eigenvector).
\end{theorem}

(* The proof of the theorem and the calculation of Jordan normal form can be found in textbook of linear algebra. And here we will mainly focus on the dynamical system problem.)

So Based on the theory in algebra, we found every map (or linear transfrom) can be represented with eigenvector and eigenvalue s.t. $A = P^{-1} J P$. Obviously, the exponent of both diagonal matrix or Jordan normal form is easy. So typically, we consider the map with this similar form.

Obviously, if we consider a matrix 
...
Henon map is a Poincare map of kicked rotator. Please study that too.
