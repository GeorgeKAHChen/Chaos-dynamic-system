...
\begin{theorem}\label{Lyapunov-exponent-chaotic} Consider map $f \in C^1(R^1)$, the orbit is \textbf{chaotic} if the Lyapunov exponent $h(x_1)$ is \textbf{greater} than zero.
\end{theorem}
...
\newpage
  \noindent Proof: Theo. \ref{schwarzian-theo}
 $\blacksquare$
\end{proof}
}
...
$$
So the problem is how to find a one-to-one between the $\tau(x)$ and $q(x)$. Even in the introduction of conjugacy in the last section, we just given the map between the tent map and logistic map and discussed less in how to find these kind of map. Here, we will used the definition of (probability) density function (PDF) in probability measure.

%[TODO] How to calculate the density function and how to find this one-to-one to find a pair of conjugacy?
...
\\\noindent \textbf{[ii]} $f|_{[-1, 0]}$ is $C^1$ and increasing, and $f|_{[0, 1]}$ is $C^1$ and decreasingl
...
Here we will introduce real analysis definition and theorem as a toolbox.
\begin{definition}\textbf{Borel set, Borel $\sigma -$algebra and Borel measure}
\end{definition}


\begin{definition}\textbf{Finite measure space, normalized/ probabilistic measure space}
\end{definition}
